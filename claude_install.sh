...
if command -v pnpm &>/dev/null && pnpm --version | grep -qE '^10\.'; then
  ...
  curl -fsSL -o "$PNPM_HOME/pnpm" \
    "https://registry.npmmirror.com/@pnpm/exe/${PNPM_VERSION}/pnpm-${PNPM_VERSION}-node-${ARCH}"
  chmod +x "$PNPM_HOME/pnpm"

  # 验证版本
  PNMP_VER=$(pnpm --version)
  if [[ "$PNMP_VER" =~ ^10\. ]]; then
    ok "pnpm ${PNMP_VER} 安装成功"
  else
    die "pnpm 版本异常：$PNMP_VER"
  fi
fi
